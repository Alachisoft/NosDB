...
This module contains sample data in CSV format which is supported by NosDB. 
This readme explains how the provided sample data (%InstallationDir%\NoSDB\samples\data\csv\northwind) can be imported into your configured database to get you started with NosDB features.
...
2. Get filenames from the respective folder in "%InstallationDir%\NoSDB\samples\data\csv\northwind\" which contains the input CSV files.
...
2. Type the following command in the context:
   ...
   & "%InstallationDir%\NoSDB\samples\data\csv\NorthWind.ps1"  (INCLUDING the & sign)
