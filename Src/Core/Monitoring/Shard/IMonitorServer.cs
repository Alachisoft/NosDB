...
namespace Alachisoft.NosDB.Core.Monitoring
{
    public interface IMonitorServer
    {
        ...
        EventViewerEvent[] GetLatestEvents();
        int GetPercentageCPUUsage();
        string GetShardNodeNIC();        
        string GetNICForIP(string ip);
        ClientProcessStats[] GetClientProcessStats(string database);

        ServerInfo[] GetRunningServers();
        ServerInfo[] GetUpdatedRunningServers();
