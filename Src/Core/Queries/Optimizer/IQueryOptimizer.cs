...
namespace Alachisoft.NosDB.Core.Queries.Optimizer
{
    ...
    {
        QueryPlan GetQueryPlan(IDmObject parsedQuery, IQuery query, IQueryStore queryStore, MetadataIndex rowEnumerator);
        ICollection<QueryPlan> GetAllPlans(IDmObject parsedQuery, IQuery query, IQueryStore queryStore);
    }
}
