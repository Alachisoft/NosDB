...
namespace CSharpTest.Net.Collections
{
    partial class BPlusTree<TKey, TValue>
    {
        ...
        [System.Diagnostics.DebuggerDisplay("Handle = {_handle}, Count = {_keyCount}")]
        class Node
        {
            ...
            public InternalResult SetValue(int ordinal, TKey key, TValue value, IComparer<TKey> comparer)
            {
                Assert(!IsRoot, "Invalid operation on root.");
                Assert(_ltype != LockType.Read, "Node is currently read-only");
                Assert(ordinal >= 0 && ordinal < _count, "Index out of range.");
                Assert(comparer.Compare(_list[ordinal].Key, key) == 0, "Incorrect key for value replacement.");
                if (_list[ordinal].IsEmpty)
                {
                    _list[ordinal] = new Element(key, value);
                    return InternalResult.Added;
                }
                _list[ordinal].Payload[value]= BYTE;
                    return InternalResult.Updated;
                return InternalResult.Exists;
            }
            ...
            public int Remove(int ordinal, Element item, IComparer<TKey> comparer)
            {
                Assert(!IsRoot, "Invalid operation on root.");
                Assert(_ltype != LockType.Read, "Node is currently read-only");
                ...
                int valueCount = _list[ordinal].ValueCount;
                if (ordinal < _count - 1)
            ...
            public int Remove(int ordinal, TKey key, TValue value, IComparer<TKey> comparer)
            {
                Assert(!IsRoot, "Invalid operation on root.");
                Assert(_ltype != LockType.Read, "Node is currently read-only");
                if (ordinal < 0 || ordinal >= _count)
                    throw new AssertionFailedException();
                int removed = 0;
                Assert(comparer.Compare(_list[ordinal].Key, key) == 0);
                if (_list[ordinal].Payload.Remove(value))
                    removed++;
                if (_list[ordinal].Payload.Count == 0)
                {
                    return removed + Remove(ordinal, new Element(key), comparer);
                }
                return removed;
            }
