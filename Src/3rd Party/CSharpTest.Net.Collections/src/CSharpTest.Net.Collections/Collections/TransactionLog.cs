...
namespace CSharpTest.Net.Collections
{
    ...
    public interface ITransactionLog<TKey, TValue> : IDisposable
    {
        ...
        /// Replay the entire log file to the provided dictionary interface
        /// </summary>
        void ReplayLog(BPlusTree<TKey, TValue> target);
        /// <summary>
        /// Replay the log file from the position provided and output the new log position
        /// </summary>
        void ReplayLog(BPlusTree<TKey, TValue> target, ref long position);
    ...
    public class TransactionLog<TKey, TValue> : ITransactionLog<TKey, TValue>
    {
        ...
        /// Replay the log file from the position provided and output the new log position
        /// </summary>
        public void ReplayLog(BPlusTree<TKey, TValue> target, ref long position)
        {
            long[] refposition = new long[] { position };
            try
            {
                ...
                {
                    if (entry.OpCode == OperationCode.Remove)
                        target.TryRemove(entry.Key, entry.Value);
                    else if (entry.OpCode == OperationCode.Update)//TODO: Log old value as well in update
                        target.TryUpdate(entry.Key, entry.Value);// [entry.Key] = entry.Value;
                    else
