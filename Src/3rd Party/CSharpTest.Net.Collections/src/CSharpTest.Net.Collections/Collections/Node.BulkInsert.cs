    ...
    partial class BPlusTree<TKey, TValue>
    {
                ...
                if (me.Count < limitMin)
                {
                    ...
                    while (me.Count < limitMin)
                    {
                        ...
                        if (me.Count + 1 == limitMin)
                        {
                            ...
                            me.Insert(0, item);
                        int removeCount = prevNode.Remove(prevNode.Count - 1, item, _keyComparer);
                        if (_hasCount)
                        {
                            _valueCount -= removeCount;
                            _keyCount--;
                        }
                    }
        ...
        private IEnumerable<KeyValuePair<TKey, TValue>> EnumerateNodeContents(NodePin root)
        {
            if (root.Ptr.IsLeaf)
            {
                ...
                {
                    var set = root.Ptr[ix].ToKeyValuePair();
                ...
                while (todo.Count > 0)
                {
                    ...
                    if (child.Ptr.IsLeaf)
                    {
                        using (child)
                        {
                            ...
                            {
                                var set = child.Ptr[ix].ToKeyValuePair();
                                var enumerator = set.Value.GetEnumerator();
                                while(enumerator.MoveNext())
                                    yield return new KeyValuePair<TKey, TValue>(set.Key, enumerator.Current.Key);
                            }
