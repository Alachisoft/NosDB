...
namespace CSharpTest.Net.Collections
{
    partial class BPlusTree<TKey, TValue>
    {
        ...
        private RemoveResult Delete(NodePin thisLock, TKey key, TValue value, NodePin parent, int parentIx)
            //where T : IRemoveValue<TKey, TValue>
        {
            ...
            if (me.Count <= minimumKeys && parent != null && !parent.Ptr.IsRoot)
            {
                if (parentIx < parent.Ptr.Count - 1)
                {
                    ...
                    thisLock.Dispose();
                    return Delete(parent, key, value, null, int.MinValue);
                }
                if (parentIx > 0)
                {
            ...
            else if (parent != null && parent.Ptr.IsRoot && me.Count == 1 && !me.IsLeaf)
            {
                using (NodePin onlyChild = _storage.Lock(thisLock, me[0].ChildNode))
                {
                    ...
                    return Delete(onlyChild, key, value, null, int.MinValue);
                }
            }
            ...
            {
                if (me[ordinal].Payload.ContainsKey(value))//(value.RemoveValue(key, me[ordinal].Payload))
                {
                    using (NodeTransaction t = _storage.BeginTransaction())
                    {
                        //Remove value
                        me = t.BeginUpdate(thisLock);
                        me.Remove(ordinal, key, value, _keyComparer);
                        t.RemoveValue(key);
                        t.Commit();
            ...
            using (NodePin child = _storage.Lock(thisLock, me[ordinal].ChildNode))
                return Delete(child, key, value, thisLock, ordinal);
        }
