        ...
        class NodeSerializer : ISerializer<Node>
        {
            ...
            void ISerializer<Node>.WriteTo(Node value, Stream stream)
            {
                ...
                for (int i = 0; i < value.Count; i++)
                {
                    ...
                    if (isLeaf)
                    {
                        _intSerializer.WriteTo(item.Payload.Count, stream);
                        foreach(var innerValue in item.Payload)
                            _valueSerializer.WriteTo(innerValue.Key, stream);
                    }
                    else
                    {
            ...
            public IEnumerable<KeyValuePair<TKey, TValue>> RecoverLeaf(Stream stream)
            {
                ...
                if (isLeaf)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        var valueSet = ReadHashSet(stream);
                        foreach(var value in valueSet)
                            yield return new KeyValuePair<TKey, TValue>(key, value.Key);
                    }
                }
            }
            ...
            Node ISerializer<Node>.ReadFrom(Stream stream)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (isLeaf)
                    {
                        ...
                        items[i] = new Element(key, ReadHashSet(stream));
                    }
                    else
            ...
            private HashVector<TValue, byte> ReadHashSet(Stream stream)
            {
                int setCount = _intSerializer.ReadFrom(stream);
                var set = new HashVector<TValue, byte>();
                for (int j = 0; j < setCount; j++)
                    set[_valueSerializer.ReadFrom(stream)]=BYTE;
                return set;
            }
        }
    }
