...
namespace CSharpTest.Net.Collections
{
    ...
    public class OrdinalList : ICollection<int>, ICollection, IEnumerable, ICloneable
    {
        ...
        public class LongList : ICollection<long>, ICollection, ICloneable
        {
            private byte[] _bits;
            ...
            /// <summary> Constructs an OrdinalList from the integer ordinals provided </summary>
            public LongList(IEnumerable<long> contents)
            {
                Clear();
                AddRange(contents);
            }
            ...
            public int Count
            {
                get
                {
                    return _bits.Aggregate(0, (current, b) => current + BitCount[b]);
                }
            }
            ...
            public long Ceiling
            {
                get { return ((((long)_bits.Length) << 3) - 1); }
                set { AllocFor(value); }
            }

            private void AllocFor(long max)
            {
                if (max >= 0)
                    max = 1 + (max >> 3);
                else if (max == -1)
            ...
            /// <summary> Adds a range of integer ordinals into the collection </summary>
            public void AddRange(IEnumerable<long> contents)
            {
                long max = long.MinValue;

                foreach (long i in contents)
                    max = Math.Max(i, max);
                ...
                foreach (long item in contents)
                {
                    long offset = item >> 3;
                    int bit = 1 << (int)(item & 0x07);
                    _bits[offset] |= unchecked((byte)bit);
                }
            }
            ...
            public long[] ToArray()
            {
                return new List<long>(this).ToArray();
            }
            ...
            /// <summary> Returns the set of items that are in both this set and the provided set </summary>
            /// <example>{ 1, 2, 3 }.IntersectWith({ 2, 3, 4 }) == { 2, 3 }</example>
            public LongList IntersectWith(LongList other)
            {
                byte[] small, big;
                big = _bits.Length > other._bits.Length ? _bits : other._bits;
                small = _bits.Length > other._bits.Length ? other._bits : _bits;
            ...
            /// <summary> Returns an enumeration of the ordinal values </summary>
            public IEnumerable<long> EnumerateFrom(long startAt)
            {
                return EnumerateRange(startAt, long.MaxValue);
            }

            /// <summary> Returns an enumeration of the ordinal values </summary>
            public IEnumerable<long> EnumerateRange(long startAt, long endAt)
            {
                long ordinal = startAt & ~0x07;
                //foreach (byte i in _bits)
                for (long ix = startAt >> 3; ix < _bits.Length; ix++)
                {
                    if (_bits[ix] != 0)
                    {
                        int i = _bits[ix];
