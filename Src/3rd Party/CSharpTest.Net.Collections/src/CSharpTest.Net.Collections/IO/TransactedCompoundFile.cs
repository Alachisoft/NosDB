...
namespace CSharpTest.Net.IO
{
    ...
    /// 
    /// ChangeLog: 1/22/2016
    /// The file system has been modified to allow nodes with size greater than a section.
    /// This is done through section chaining. As an advice, if, O poor developer, you are given the task of
    /// optimizing this file system again, tell them to make a new or more proper file system, or make one yourself,
    /// otherwise you're straight outta luck, because this is a hellish piece of code.
    /// </summary>
    public class TransactedCompoundFile : ITransactable
    {
        ...
        const int BlockHeaderSize = 20; //length + CRC
        ...
        private const int OffsetOfSectionChain = 16;
        ...
        private bool LoadSections(Stream stream, LoadFrom from)
        {
            ...
            {
                if (from == LoadFrom.Either && FileSection.TryLoadSection(stream, false, i, BlockSize,this, out sections[i]))
                    continue;

                if (!FileSection.TryLoadSection(stream, from != LoadFrom.FirstBlock, i, BlockSize,this, out sections[i]))
                    return false;
            }
        ...
        {
            FileSection n = new FileSection(_sections.Length, BlockSize, this, false);
            lock (_sync)
            {
        ...
        private int ReserveWholeSection()
        {
            FileSection n = new FileSection(_sections.Length, BlockSize, this, true);
            lock (_sync)
            {
                n.Commit(_fput, false);
                n.Commit(_fput, true);
        ...
        private Queue<int> ReclaimSections(int sectionsNeeded)
        {
            Queue<int> listOfReclaimableSections = new Queue<int>();
            //foreach (var fileSection in _sections)
            //{
            //    if (fileSection.FreeBlockList.Count >= BlocksPerSection - 2)
            //    {
            //        allocationList.Add(new Allocation(fileSection.SectionIndex + 1, fileSection.FreeBlockList.Count));
            //    }
            //    if (allocationList.Count >= sectionsNeeded)
            //        return allocationList;
            //}
            for (int i = 0; i < _sections.Length; i++)
            {
                if (_sections[i].FreeBlockList.Count >= BlocksPerSection - 2)
                {
                    listOfReclaimableSections.Enqueue(i);
                }
                if (listOfReclaimableSections.Count >= sectionsNeeded)
                    break;
            }
            return listOfReclaimableSections;
        } 

        private List<Allocation> TakeAllocations(int blocksNeeded)
        {
            var allocationSet = new List<Allocation>();
            int sectionsNeeded = Convert.ToInt32(Math.Ceiling(Math.Max(1f, (float)blocksNeeded/(BlocksPerSection - 2))));
            lock (_sync)
            {
                if (sectionsNeeded > 1)
                {
                    var reclaimed = ReclaimSections(blocksNeeded);

                    for (int i = 1; i <= sectionsNeeded; i++)
                    {
                        if (reclaimed.Count > 0)
                        {
                            var sectionNumber = reclaimed.Dequeue();
                            if (i != sectionsNeeded)
                            {
                                allocationSet.Add(new Allocation(_sections[sectionNumber].SectionIndex*BlocksPerSection + 1, _sections[sectionNumber].FreeBlockList.Count));

                                foreach (var id in _sections[sectionNumber].FreeBlockList)
                                {
                                    _freeBlocks.Remove(id);
                                }

                                _sections[sectionNumber].FreeBlockList.Clear();
                            }
                            else
                            {
                                int lastBlocks = blocksNeeded % (BlocksPerSection - 2);
                                if (lastBlocks == 0)
                                    lastBlocks = BlocksPerSection - 2;
                                TakeAllocationsForSingle(lastBlocks, allocationSet);
                            }
                        }
                        else
                        {
                            if (i != sectionsNeeded)
                                allocationSet.Add(new Allocation(ReserveWholeSection(), BlocksPerSection - 2));
                            else
                            {
                                int lastBlocks = blocksNeeded % (BlocksPerSection - 2);
                                int newSection = ReserveWholeSection();
                                allocationSet.Add(new Allocation(newSection, lastBlocks));

                                int lastFree = newSection + BlocksPerSection - 2;
                                for (int j = newSection + lastBlocks; j <= lastFree; j++)
                                {
                                    _freeBlocks.Add(j);
                                    _sections[_sections.Length - 1].FreeBlockList.Add(j);
                                }
                            }
                        }
                ...
                {
                    TakeAllocationsForSingle(blocksNeeded,allocationSet);
                }
            }
        ...
        private void TakeAllocationsForSingle(int blocksNeeded, List<Allocation> allocationSet)
        {
            bool rescan = false;
            bool resized = false;
            int startingFrom = _prevFreeBlock;
            ...
            while (true)
            {
                ...
                foreach (int free in _freeBlocks.EnumerateRange(startingFrom, endingBefore))
                {
                    ...
                    if (found == blocksNeeded)
                    {
                        ...
                        for (int i = start; i <= free; i++)
                        {
                            int sectionNumber = (int)i / BlocksPerSection;
                            _sections[sectionNumber].FreeBlockList.Remove(i);
                            _freeBlocks.Remove(i);
                        }
                        ...
                        //blockId |= ((uint)Math.Min(16, blocksNeeded) - 1 << 28) & 0xF0000000u;
                        //return blockId;
                        allocationSet.Add(new Allocation(start, found));
                        return;
                    }
                }
                if (resized)
                    throw new ArgumentOutOfRangeException("length: " + blocksNeeded);
        ...
        private void FreeBlocks(BlockRef block)
        {
            ...
            if (free > 0)
            {
                ...
                {
                    int headerSize, length;
                    _sections[block.Section].Read(ref block, true, _fget, out chain, out headerSize, out length);
                ...
                {
                    var blockId = free + i;
                    _sections[block.Section].FreeBlockList.Add(blockId);
                    _freeBlocks.Add(blockId);
                }
                if (chain != 0)
                {
                    block = new BlockRef(chain, BlockSize);
                    FreeBlocks(block);
                }
            }
        }
        ...
        {
            HandleRef href = new HandleRef(handle, BlockSize);
            bool handleZero = false, section = false, freehandle = false;
            if ((handleZero = handle == 0) || (section = href.Section >= _sections.Length) ||
                (freehandle = _freeHandles.Contains((int) handle)))
                throw new ArgumentOutOfRangeException(
                    "handle, HandleZero=" + handleZero + ",SectionLength=" + section + ",FreeHandle=" + freehandle);

            uint oldblockId = _sections[href.Section][href.Offset];
            int sectionsNeeded =
                Convert.ToInt32(Math.Ceiling(Math.Ceiling((double) length/BlockSize)/(BlocksPerSection - 2)));
            int blocksNeeded = Math.Max(1, (length + BlockHeaderSize*sectionsNeeded + BlockSize - 1)/BlockSize);
            //if (blocksNeeded > BlocksPerSection-2)
            //    throw new ArgumentOutOfRangeException("length");


            List<Allocation> allocations = TakeAllocations(blocksNeeded);
            int blocksWritten = 0;
            int bytesWritten = offset;
            uint firstBlock = 0;
            for (int i = 0; i < allocations.Count; i++)
            {
                uint blockId = (uint) allocations[i].Offset |
                               ((uint) Math.Min(16, blocksNeeded - blocksWritten) - 1 << 28) & 0xF0000000u;
                var block = new BlockRef(blockId, BlockSize, Math.Min(1022, blocksNeeded - blocksWritten));
                ...
                if (allocations.Count == 1)
                {
                    ...
                    lock (_sync)
                    {
                        _sections[block.Section].Write(block, _fput, bytes, offset, length, 0);
                ...
                lock (_sync)
                {
                    int newLength = allocations[i].BlockCount*BlockSize - BlockHeaderSize;
                    uint nextChain;
                    if (i == (allocations.Count - 1))
                    {
                        newLength = length - bytesWritten;
                        nextChain = 0;
                    }
                    else
                    {
                        nextChain =
                            (uint)
                                (allocations[i + 1].Offset |
                                 ((uint)
                                     Math.Min(16, blocksNeeded - Math.Ceiling((double)(bytesWritten + newLength + BlockHeaderSize)/BlockSize)) -
                                  1 << 28) & 0xF0000000u);
                    }
                    _sections[block.Section].Write(block, _fput, bytes, bytesWritten, newLength, nextChain);
                    if (i == allocations.Count-1)
                    {
                        _sections[href.Section].SetHandle(_fcommit, href.Offset, firstBlock);
                    }
                    bytesWritten += newLength;
                    blocksWritten += allocations[i].BlockCount;
                }
            }
            if (oldblockId != 0)
                FreeBlocks(new BlockRef(oldblockId, BlockSize));
        ...
        public Stream Read(uint handle)
        {
            ...
            while (blockId != 0)
            {
                ...
                byte[] buffer = _sections[block.Section].Read(ref block, false, _fget, out blockId, out headerSize,
                    out length);
                ms.Write(buffer, headerSize, length);
            }
            //BlockRef block = new BlockRef(blockId, BlockSize);
            //Stream stream =  _sections[block.Section].Read(ref block, false, _fget, out chain);
            //while (chain != 0)
            //{
            //    block = new BlockRef(chain, BlockSize);
            //    Stream newStream = _sections[block.Section].Read(ref block, false, _fget, out chain);
        ...
        class FileSection
        {
            ...
            readonly TransactedCompoundFile _parent;
            private HashSet<int> _freeBlocks;

            private bool _isDirty;

            public HashSet<int> FreeBlockList { get { return _freeBlocks; } }
            ...
            private FileSection(int sectionIndex, int blockSize, bool create, TransactedCompoundFile parent, bool reserveAll)
            {
                ...
                _freeBlocks = new HashSet<int>();
                if (!reserveAll)
                {
                    int baseHandle = unchecked(BlocksPerSection*_sectionIndex);
                    for (int i = baseHandle + 1; i < baseHandle + BlocksPerSection - 1; i++)
                        _freeBlocks.Add(i);
                }
            ...
            public FileSection(int sectionIndex, int blockSize, TransactedCompoundFile parent, bool reserveAll)
                : this(sectionIndex, blockSize, true, parent, reserveAll)
            { }

            public static bool TryLoadSection(Stream stream, bool alt, int sectionIndex, int blockSize, TransactedCompoundFile parent, out FileSection section)
            {
                section = new FileSection(sectionIndex, blockSize, false, parent, false);
            ...
            public void Write(BlockRef block, FPut fput, byte[] bytes, int offset, int length, uint chain)
            {
                var blockdata = new byte[length+BlockHeaderSize];
                ...
                PutUInt32(blockdata, OffsetOfSectionChain, chain);
            ...
            public byte[] Read(ref BlockRef block, bool headerOnly, FGet fget, out uint chain, out int headerSize, out int length)
            {
                bool retry;
                byte[] bytes;
                int readBytes;
                do
                {
                    ...
                    chain = GetUInt32(bytes, OffsetOfSectionChain);
            ...
            public void GetFree(ICollection<int> freeHandles, ICollection<int> usedBlocks, FGet fget)
            {
                ...
                for (int handle = 1; handle < BlocksPerSection - 1; handle++)
                {
                    ...
                    else
                    {
                        ...
                        GetFreeBlockChain(freeHandles, usedBlocks, fget, block);
                        ...
                        //    uint chain2 = GetUInt32(header, OffsetOfSectionChain);
                        //}
                        ...
                        //uint chain = 0;
                        //int headerSize, length;
                        //_parent._sections[block.Section].Read(ref block, true, _parent._fget, out chain, out headerSize,
                        //    out length);
                        //if (chain != 0)
                        //{
                        //    block = new BlockRef(chain, BlockSize); blockId = (int)block.Identity & 0x0FFFFFFF;
                        //    if (block.Count == 16)
                        //    {
                        //        long position = (long)BlocksPerSection * BlockSize * block.Section;
                        //        position += BlockSize * block.Offset;
            ...
            private void GetFreeBlockChain(ICollection<int> freeHandles, ICollection<int> usedBlocks, FGet fget,
                BlockRef block)
            {
                int blockId = (int) block.Identity & 0x0FFFFFFF;
                ...
                if (block.Count == 16)
                {
                    ...
                    uint chain = GetUInt32(header, OffsetOfSectionChain);
                    if (chain != 0)
                    {
                        var chainBlock = new BlockRef(chain, BlockSize);
                        blockId = (int) block.Identity & 0x0FFFFFFF;
                        GetFreeBlockChain(freeHandles, usedBlocks, fget, chainBlock);
                    }
                }
                ...
                for (uint i = 0; i < block.ActualBlocks; i++)
                {
                    _parent._sections[block.Section]._freeBlocks.Remove(blockId);
                    usedBlocks.Add(blockId++);
                }
            }
        ...
        public class Allocation : IComparable
        {
            private int offset;
            private int blockCount;

            public Allocation(int _offset, int _blockCount)
            {
                offset = _offset;
            ...
            {
                var allocation = obj as Allocation;
                if (allocation != null)
                    return offset.CompareTo(allocation.offset);
