...
namespace Alachisoft.NosDB.Common.Net
{
	...
	public class Address : ICloneable, IComparable,ICompactSerializable
	{
		...
		public Address(string i, int p)
		{
			...
			{
				//Trace.warn("failed to get " + i + ':' + p + ", using \"127.0.0.1\", exception: " + e.StackTrace);
                //Trace.error("Address.Address()", e.ToString());
				try
				{
				...
				{
                    //Trace.error("IPAddress.Address():2", ex.ToString());
				}
			}
			port = p;
		}
	    ...
	    public string ip
	    {
	        get
	        {
	            if (ip_addr != null) return ip_addr.ToString();
	            return "";
	        }
            set
            {
		...
		static public bool isMulticastAddress(string ipaddr)
		{
			try
			{
				...
				//					System.Net.Dns.Resolve(ipaddr).AddressList[0];
				//				return isMulticastAddress(ip_addr);
