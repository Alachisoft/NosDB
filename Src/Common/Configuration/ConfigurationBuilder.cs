...
//using Alachisoft.NosDB.Exceptions;
#endif
namespace Alachisoft.NosDB.Common.Configuration
{
    ...
    /// see <see cref="Alachisoft.NosDB.Common.Configuration.ConfiugrationAttributes."/>. Below is example
    ...
    public class ConfigurationBuilder
    {
        ...
        private void ReadConfiguration(string file, string path)
        {
            ...
            _lastLoadedConfiugration = new ArrayList();
            string fileId = path + file;
            if (!File.Exists(fileId))
            {
                //throw new Exception("File " + fileId + " not found");
                using (FileStream fs = File.Create(fileId, 10, FileOptions.None))
                {
            ...
            {
                new Exception("Can not open " + fileId + " Error:" + e.ToString());
            }
        ...
        private void PopulateConfiugrationObject(Object config, XmlNode node)
        {
            ...
            for (int i = 0; i < nodeList.Count; i++)
            {
                ...
                if (sectionType != null)
                {
                    ...
                    {
                        //Object sectionConfig = GetConfigSectionObject(config, sectionNode.Name);
                ...
                if (objProps != null)
                {
                    for (int i = 0; i < objProps.Length; i++)
                    {
                        ...
                        if (customAttribs != null && customAttribs.Length > 0)
                        {
                            ...
                            {
                                throw new Exception("Duplicate cache entries in " + config);
                            }
                        }
                        ...
                        if (customAttribs != null && customAttribs.Length > 0)
                        {
                            ...
                            {
                                throw new Exception("Duplicate cache entries in " + config.ToString());
                            }
                        }
                    }
                }
            ...
            if (objProps != null)
            {
                for (int i = 0; i < objProps.Length; i++)
                {
                    ...
                    if (customAttribs != null && customAttribs.Length > 0)
                    {
                        ...
                        {
                            throw new Exception("Can not set the value  " + configAttrib.AttributeName + " Errror :" + e.ToString());
                        }
