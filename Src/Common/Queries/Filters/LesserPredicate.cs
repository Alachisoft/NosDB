...
{
    public class LesserPredicate: TerminalPredicate
    {
        private AttributeValue _value;

        public LesserPredicate( IIndex sourceIndex, AttributeValue value)
        {
            source = sourceIndex;
            _value = value;
        }

        public LesserPredicate(IIndex sourceIndex, AttributeValue value, bool isInverse) : this(sourceIndex, value)
        {
            IsInverse = isInverse;
        }
                    ...
                    if (!max.Equals(min))
                    {
                        try
                        {
                            ...
                            if (_value.DataType == FieldDataType.Number)
                            {
                                ...
                                if (!IsInverse)
                                    return source.ValueCount * ((value - dmin) / (dmax - dmin));
                                return source.ValueCount * ((dmax - value) / (dmax - dmin)) + 1;
                            }
                        }
                        catch
                        {
                ...
                if (_value.DataType.Equals(FieldDataType.Array))
                {
                    ...
                    if (!arrayMax.Equals(arrayMin))
                    {
                        try
                        {
                            ...
                            if (!IsInverse)
                                return source.ValueCount * ((value - armin) / (armax - armin));
        ...
        {
            output.Write("LesserPredicate:{");
            base.Print(output);
            output.Write(",LessThan=" + (_value != null ? _value.ValueInString : "null"));
            output.Write("}");
        }
    }
}
