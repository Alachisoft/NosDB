...
namespace Alachisoft.NosDB.Common.Util
{
    public class RegHelper
    {
        ...
        public static string ROOT_KEY = @"Software\Alachisoft\NosDB";
        public static string APPBASE_KEY = @"\NosDB Manager";
        public static string REMOTEAUTH_KEY = @"Software\Alachisoft\NosDB\NosDBMonitor\";
        public static string MANAGER_OPTIONS = APPBASE_KEY + @"\Options";
        public static string MONITOR_ROWS_BASE = @"Software\Alachisoft\NosDB\NosDB Manager\Options";
        public static string SSL_KEY = Path.Combine(ROOT_KEY, "Security");
        ...
        {
            return GetRegValueInternal("UserInfo", "licensekey", prodId);
        ...
        static public int GetRegValues(string keypath, Hashtable ht, short prodId)
        {
            ...
            try
            {
                RegistryKey root = Registry.LocalMachine.OpenSubKey(keypath);
                if (root != null)
                {
        ...
        static public int GetRegValuesFromCurrentUser(string keypath, Hashtable ht, short prodId)
        {
            ...
            try
            {
                RegistryKey root = Registry.CurrentUser.OpenSubKey(keypath);
                if (root != null)
                {
                    string[] keys = root.GetValueNames();
                    for (int i = 0; i < keys.Length; i++)
        ...
        static private string GetRegValueInternal(string section, string key, short prodId)
        {
            try
            {
                ...
                DBRegistryDll.GetRegVal(regVal, sbSection, sbKey, sbDefaultVal, prodId);
        ...
        static private void GetRegValuesInternalWow64(string keypath, Hashtable ht, short prodId) //Added by  Aug-08,08
        {
            try
            {
                ...
                StringBuilder sbDefaultVal = new StringBuilder("");
                DBRegistryDll.GetRegKeys(regVal, sbSection, sbKey, sbDefaultVal, prodId);
        ...
        static private void GetBooleanRegValuesInternalWow64(string keypath, Hashtable ht, short prodId) //Added by  Aug-08,08
        {
            try
            {
                StringBuilder regVal = new StringBuilder(2048);
