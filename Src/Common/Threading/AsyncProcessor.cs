...
namespace Alachisoft.NosDB.Common.Threading
{
    ...
    public class AsyncProcessor
    {
        ...
        /// Constructor
        /// </summary>
        public AsyncProcessor(ILogger NLog)
            : this()
        {
            this.NLog = NLog;
            //this.nTrace = nTrace;
        ...
        {
          //  NLog.Warn("AsyncProcessor", "WindUp Task Started.");

            if (_eventsHi != null)
            {
        //        NLog.Warn("AsyncProcessor", "Async operation(s) Queue Count: " + _eventsHi.Count);
            }
         ...
         //   NLog.Warn("AsyncProcessor", "WindUp Task Ended.");
        }

        public void WaitForShutDown(long interval)
        {
        //    NLog.Warn("AsyncProcessor", "Waiting for  async process queue shutdown task completion.");
            if (interval > 0)
            {
                bool waitlock = false;
                lock (_shutdownMutex)
                ...
                {
          //          if(_eventsHi.Count > 0)
             //           NLog.Warn("AsyncProcessor", "Remaining Async operations in queue: " + _eventsHi.Count);
                }
            }

          // NLog.Warn("AsyncProcessor", "Shutdown task completed.");
        }
        ...
        public void Start()
        {
            ...
            catch (Exception ex)
            {
                if (NLog != null)
                {
            //        NLog.Error("AsyncProcessor.Start()", ex.Message);
                }
            }
        }
        ...
        public void Stop()
        {
            ...
            catch (Exception ex)
            {
                if (NLog != null)
                {
            //        NLog.Error("AsyncProcessor.Stop()", ex.Message);
                }
            }
        }
            ...
            while (_started)
            {
                ...
                catch (ThreadAbortException e)
                {
                  ...
                  //      NLog.Flush();
                    ...
                    //if (nTrace != null) nTrace.error("AsyncProcessor.Run()", e.ToString());
                    break;
                }
                ...
                { 
                    ...
                    if (exceptionString != "ChannelNotConnectedException" && exceptionString != "ChannelClosedException")
                    {
                        if (NLog != null)
                        {
             //               NLog.Error("AsyncProcessor.Run()", exceptionString);
